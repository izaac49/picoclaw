	...
	if err == nil {
		...
		go func(cid int64, mid int) {
			...
			for {
				...
				case <-ticker.C:
					i++
					text := fmt.Sprintf("Thinking%s 🦞", dots[i%len(dots)])
					c.bot.EditMessageText(thinkCtx, tu.EditMessageText(tu.ID(chatID), mid, text))
				}
			}
		}(chatID, pID)
	}
...
func (c *TelegramChannel) downloadFileWithInfo(file *telego.File, ext string) string {
	...
	return utils.DownloadFile(url, file.FilePath+ext, utils.DownloadOptions{LoggerPrefix: "telegram"})
}
